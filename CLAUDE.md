...
Seven agent roles (surveyor, orchestrator, researcher, computer, reviewer, deep critic, compressor, formatter) take turns in a main loop. The surveyor runs once before the main loop (and can be re-invoked mid-loop). Three core agents handle evidence production and review:
...
- `submit_review` (reviewer) uses `stop_after_round` similarly; `process_response` creates `ReviewResult` on target WH
...
- **Reviewer** — adversarial review via ReviewerToolExecutor; gets focused context (WH + evidence + light state); produces ReviewResult with verdict stored on WH
