...
- Orchestrator tools (`orchestrator_tools.py`) mutate `self.research_state` directly; 9 mutation tools plus 4 dispatch tools (`dispatch_researcher`, `dispatch_computer`, `dispatch_reviewer`, `request_termination`); dispatch tools set `stop_after_round = True` to end the orchestrator loop; `dispatch_reviewer` enforces WH-only targets; `add_hypothesis` requires `from_rq` (every WH must originate from an RQ with gathered evidence) and auto-copies evidence from RQ to new WH; `promote_hypothesis` checks `h.review.verdict == "VERIFIED"` and established dependencies (mainly a fallback — the engine auto-promotes after VERIFIED reviews via `_auto_promote`)
