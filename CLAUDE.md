...
Eight agent roles (surveyor, planner, orchestrator, researcher, computer, reviewer, deep critic, formatter) take turns in a main loop. The surveyor runs once before the main loop (and can be re-invoked mid-loop), and the planner runs once after the surveyor at iteration 0. Three core agents handle evidence production and review:
