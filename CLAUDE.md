...
- Orchestrator tools (`orchestrator_tools.py`) mutate `self.research_state` directly; 10 tools total including `append_note` for research notes and `update_section` supporting Conventions, Situation Assessment, and Strategy; two-phase dispatch gate in `_set_next_task` rejects if entity-creating mutations occurred in same response; `add_hypothesis` requires `from_rq` (every WH must originate from an RQ with gathered evidence) and auto-copies evidence from RQ to new WH; `promote_hypothesis` checks `h.review.verdict == "VERIFIED"` and established dependencies (mainly a fallback — the engine auto-promotes after VERIFIED reviews via `_auto_promote`)
