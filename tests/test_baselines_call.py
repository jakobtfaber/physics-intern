...
def test_returns_expected_dict_shape():
    ...
    result = run_baseline_call(
        provider, config,
        system="sys", user_message="q", agent_name="test",
    )

    assert set(result.keys()) == {
        "tokens", "duration_s", "cost_usd", "stop_reason", "response_text",
    }
    assert result["response_text"] == "hello"
    assert result["stop_reason"] == "end_turn"
    assert result["tokens"] == {
        "input": 10, "output": 5, "reasoning": 0, "answer": 0,
    }
...
def test_invokes_max_tokens_continuation_when_truncated():
    ...
    cont_response = _make_response(
        text="partial and then more", stop_reason="end_turn",
