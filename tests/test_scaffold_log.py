"""Tests for SCAFFOLDING_LOG.jsonl instrumentation."""
...
class TestLogScaffoldEvent:
    ...
    def test_creates_valid_jsonl(self, tmp_path):
        ...
        logfile = tmp_path / "SCAFFOLDING_LOG.jsonl"
    ...
    def test_appends_multiple(self, tmp_path):
        ...
        lines = (tmp_path / "SCAFFOLDING_LOG.jsonl").read_text().strip().split("\n")
        assert len(lines) == 2
    ...
    def test_empty_detail(self, tmp_path):
        ...
        entry = json.loads((tmp_path / "SCAFFOLDING_LOG.jsonl").read_text().strip())
        assert entry["detail"] == ""
...
class TestValidatePostIntegrationLogs:
    ...
    def test_violations_are_logged(self, tmp_path):
        """Set up workspace with a phantom reference, verify SCAFFOLDING_LOG.jsonl gets Layer 4 event."""
        ...
        logfile = ws.root / "SCAFFOLDING_LOG.jsonl"
        assert logfile.exists()
...
class TestBudgetOverrideLogs:
    ...
    def test_budget_override_logs_event(self, tmp_path):
        ...
        logfile = ws_dir / "SCAFFOLDING_LOG.jsonl"
        assert logfile.exists()
        entry = json.loads(logfile.read_text().strip())
        assert entry["layer"] == 5
        assert entry["event"] == "p1_budget_override"
