#!/usr/bin/env python3
"""Two-step LLM baseline for OpenDirac problems.
...
Usage:
    ...
    uv run python -m open_dirac.two_steps problems/critpt/quantum_error_correction_main.yaml -o result.md
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        prog="open_dirac.two_steps",
        description="Two-step LLM baseline (critpt parsing=False) for OpenDirac problems.",
    )
    ...
    if not answer_template:
        print(
            ...
            "open_dirac.one_shot for problems without a template.",
            file=sys.stderr,
        )
        sys.exit(1)
