...
def _main_fresh(args) -> None:
    ...
    if args.workspace_dir is None:
        ...
        run_name = f"{timestamp}_{args.problem.stem}_{safe_model}_physics_intern"
...
def _main_resume(args) -> None:
    ...
    for key in (
        ...
        if value is not None:
            overrides[key] = value
    engine = PhysicsIntern.resume(workspace_path, config_overrides=overrides or None)
    engine.run()
